...
1. Create the data directory (`~/.komodo/VRSC/` on Linux, `~/Library/Application Support/Komodo/VRSC/` on macOS, `%AppData%\Komodo\VRSC\` on Windows)
...
> When `blocks` equals `longestchain`, your node is fully synchronized.
