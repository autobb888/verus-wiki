...
> Sign messages and authenticate as a VerusID entirely offline — no running daemon, no RPC connection, no network access during signing.
...
Combining offline signing with the LoginConsent protocol (see [VerusID Login: A Developer's Guide](./verusid-login-guide.md)):
...
- [@bitgo/utxo-lib (VerusCoin fork)](https://github.com/VerusCoin/BitGoJS) — `IdentitySignature` class, `signMessageOffline()`
- [verus-typescript-primitives](https://github.com/VerusCoin/verus-typescript-primitives) — `LoginConsentRequest`/`Response`, `VdxfUniValue`, identity transaction classes
- [bitcoinjs-message](https://github.com/bitcoinjs/bitcoinjs-message) — `sign()` / `verify()` with custom message prefix support
